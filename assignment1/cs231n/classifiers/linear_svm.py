...
def svm_loss_naive(W, X, y, reg):
    ...
    # scale gradient ovr the number of samples
    dW /= num_train
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    num_train = X.shape[0]
    scores = X @ W  # of shape (num_train, num_classes)
    scores_correct = scores[range(num_train), y][:, np.newaxis]
    margins = np.maximum(0, scores - scores_correct + 1)
    margins[range(num_train), y] = 0
    loss = margins.sum() / num_train + reg * np.sum(W**2)
    ...
    dW = X.T @ dW / num_train + 2 * reg * W   # gradient w.r.t. W
